// This component is working on  the updated sheet that has three separate columns, I have created a logic that merges first two subsequent columns (number and factor) and make it like we had in our previous sheet (result of requirement). then rest of the logic is same, However, I haven't made the changes in the matching logic (in useEffect) so it wouldn't give the correct result. If you wanna proceed with this logic you will have to break the matched data into two separate parts and match the break data with first two subsequent columns (number and factor).
...
const Update1 = () => {
    ...
    useEffect(() => {
        const filePath = '/assets/Konsilliar (1).xlsx';
    ...
    // This gives us all the numerical data along with the operators for the selected requirement (e.g.  > 100, > 30)
    const numericalValues = selectedRequirement.requirement &&
        excelData.slice(selectedRequirement.index)
            .reduce((acc, row, index, array) => {
                if (row.Requirement === selectedRequirement.requirement || row.Requirement === "") {
                    const originalIndex = excelData.findIndex((item) => item['Quantitative/Numerical Result of Requirement '] === row['Quantitative/Numerical Result of Requirement ']);
                    acc.push(
                        <Dropdown.Item
                            key={row['Quantitative/Numerical Result of Requirement '] + index}
                            eventKey={row['Quantitative/Numerical Result of Requirement ']}
                            data-index={originalIndex}
                            style={{ whiteSpace: 'normal', wordWrap: 'break-word' }}
                        >
                            {row['Quantitative/Numerical Result of Requirement ']}
                        </Dropdown.Item>
                    );
                } else {
                    array.splice(0);
    ...
    // This gives us all the data from the Factor of numerical Requirement column for the selected requirement (e.g.  Urin-Osmolalität in mOsm/kg, Urin Natriumkonzentration in mml/l)
    const factorOfNumericalRequirement = selectedRequirement.requirement &&
        excelData.slice(selectedRequirement.index)
            .reduce((acc, row, index, array) => {
                if (row.Requirement === selectedRequirement.requirement || row.Requirement === "") {
                    const originalIndex = excelData.findIndex((item) =>
                        item['Factor of numerical Requirement (in the same order as the given numerical result of the requirement)'] === row['Factor of numerical Requirement (in the same order as the given numerical result of the requirement)']);
                    acc.push(
                        <Dropdown.Item
                            key={row['Factor of numerical Requirement (in the same order as the given numerical result of the requirement)'] + index}
                            eventKey={row['Factor of numerical Requirement (in the same order as the given numerical result of the requirement)']}
                            data-index={originalIndex}
                            style={{ whiteSpace: 'normal', wordWrap: 'break-word' }}
                        >
                            {row['Factor of numerical Requirement (in the same order as the given numerical result of the requirement)']}
                        </Dropdown.Item>
                    );
                } else {
                    array.splice(0);
    ...
    // This gives us all the data from Non numerical requirement column for the selected requirement (e.g. Urinosmolalität > Plasmaosmolalität?)
    const nonNumericalRequirement = selectedRequirement.requirement &&
        excelData.slice(selectedRequirement.index)
            .reduce((acc, row, index, array) => {
                if (row.Requirement === selectedRequirement.requirement || row.Requirement === "") {
                    const originalIndex = excelData.findIndex((item) =>
                        item['Non numerical requirement'] === row['Non numerical requirement']);
                    acc.push(
                        <Dropdown.Item
                            key={row['Non numerical requirement'] + index}
                            eventKey={row['Non numerical requirement']}
                            data-index={originalIndex}
                            style={{ whiteSpace: 'normal', wordWrap: 'break-word' }}
                        >
                            {row['Non numerical requirement']}
                        </Dropdown.Item>
                    );
                } else {
                    array.splice(0);
    ...
    console.log("numericalValues", numericalValues);
    console.log("factorOfNumericalRequirement", factorOfNumericalRequirement);
    console.log("nonNumericalRequirement", nonNumericalRequirement);

    // Here we split the numerical data so we get separate array values for each number
    const cleanedNumericalValues = numericalValues && numericalValues.reduce((acc, item) => {
        if (item.props.eventKey && item.props.eventKey !== '') {
            const values = item.props.eventKey.split(',').map(value => value.trim());
    ...
    console.log("cleanedNumericalValues", cleanedNumericalValues);

    // Here we split the Factor Of Numerical Requirement so we get separate array values for each factor
    const cleanedFactorOfNumericalRequirement = factorOfNumericalRequirement && factorOfNumericalRequirement.reduce((acc, item) => {
        if (item.props.eventKey && item.props.eventKey !== '') {
            const values = item.props.eventKey.split(',').map(value => value.trim());
            values.forEach(value => {
                acc.push(value);
    ...
    console.log("CleanedFactorOfNumericalRequirement", cleanedFactorOfNumericalRequirement);

    // Here we split the Factor Of Numerical Requirement so we get separate array values for each factor (however this is not needed)
    const cleanedNonNumericalRequirement = nonNumericalRequirement && nonNumericalRequirement.reduce((acc, item) => {
        if (item.props.eventKey && item.props.eventKey !== '') {
            acc.push(item.props.eventKey)
        }
        return acc;
    }, []);

    console.log("cleanedNonNumericalRequirement", cleanedNonNumericalRequirement);

    // Here we add the cleanedNumericalValues with cleanedFactorOfNumericalRequirement so it looks like the data that we had in the previous sheet (Result of requirement). e.g. Urin-Osmolalität  > 100 mOsm/kg, Urin Natriumkonzentration > 30 mml/l 
    const arr = cleanedFactorOfNumericalRequirement && cleanedFactorOfNumericalRequirement.map((factor, index) => {
        const cleanedValue = cleanedNumericalValues[index];
        if (factor.includes(' in ')) {
            return factor.replace(' in ', ` ${cleanedValue}`);
        } else {
            return `${factor} ${cleanedValue}`;
        }
    });
    ...
    const cleanedArray = selectedRequirement.requirement && arr.map((items) => {
        const wordsBeforeSymbols = items.split(/(>|≤|<|≥)/);
        const cleanedThreshold = wordsBeforeSymbols[2]?.replace(/[^\d.]/g, '');
        const unit = wordsBeforeSymbols[2]?.replace(/\d|\./g, '').trim();
        return [wordsBeforeSymbols[0], wordsBeforeSymbols[1], cleanedThreshold, unit];
    });
    ...
    const splitedValues = arr && arr.map((items) => {
        const wordsBeforeSymbols = items.split(/(>|≤|<|≥)/);
        const cleanedThreshold = wordsBeforeSymbols[2]?.replace(/[^\d.]/g, '');
        const unit = wordsBeforeSymbols[2]?.replace(/\d|\./g, '').trim();
        return ([wordsBeforeSymbols[0], wordsBeforeSymbols[1], cleanedThreshold, unit]);
    ...
    const splitedArray = Array.from(new Set(splitedValues));
